...
function unpackH(
    bytes memory hBytes
) pure returns (bool success, uint256[][] memory h) {
...
) pure returns (uint256[][] memory coefficients) {
    uint256 coeffBits;
 ...
 */
function dilithiumCore1(
    Signature memory signature
) pure returns (bool foo, uint256 normH, uint256[][] memory h, uint256[][] memory z) {
