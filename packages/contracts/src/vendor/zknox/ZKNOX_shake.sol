 ...
 */
function shakeAbsorb(
    uint256 i,
    uint8[200] memory buf,
    uint64[25] memory state,
    bytes memory input
) pure returns (uint256 iout, uint8[200] memory bufout, uint64[25] memory stateout) {
...
function shakeUpdate(
    CtxShake memory ctx,
    bytes memory input
) pure returns (CtxShake memory ctxout) {
 ...
 */
function shakeSqueeze(
    CtxShake memory ctx,
    uint256 n
) pure returns (CtxShake memory ctxout, bytes memory) {
 ...
 */
function shakePermute(
    uint8[200] memory buf,
    uint64[25] memory state
) pure returns (uint8[200] memory buffer, uint64[25] memory stateout) {
    assembly {
...
function shakeDigest(
    CtxShake memory ctx,
    uint256 size8
) pure returns (bytes memory output) {
