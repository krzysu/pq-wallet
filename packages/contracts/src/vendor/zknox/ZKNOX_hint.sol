...
int256 constant GAMMA_2 = 95_232;
int256 constant _2_GAMMA_2 = 190_464;
...
function reduceModPm(
    int256 r0
) pure returns (int256 res) {
    res = r0 % _2_GAMMA_2;
    if (res > GAMMA_2) {
        res = res - _2_GAMMA_2;
    }
...
function decompose(
    uint256 r
) pure returns (int256 r1, int256 r0) {
 ...
 */
function useHintDilithium(
    uint256[][] memory h,
    uint256[][] memory r
) pure returns (bytes memory hint) {
    hint = new bytes(768);
